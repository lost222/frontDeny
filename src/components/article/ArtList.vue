...
export default {
  ...
  methods: {
    // 获取文章列表
    async getArtList() {
      const { data: res } = await this.$http.get('feeds', {
        params: {
      ...
      this.Artlist = res.feeds
      this.pagination.total = res.total

    },
    // 获取分类
    async getCateList() {
      const { data: res } = await this.$http.get('favs')
    ...
    async getCateArt(favname) {
      const { data: res } = await this.$http.get(`feed/list/`, {
        params: {
          pagesize: this.queryParam.pagesize,
          pagenum: this.queryParam.pagenum,
          fav : favname,
